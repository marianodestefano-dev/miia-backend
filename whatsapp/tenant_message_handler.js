...
// === V2 (C-386 sesión 3) — wire-in voice_seed + mode_detectors + splitter + emoji injector ===
// Scope: SOLO MIIA Personal (UID bq2BbtCVF8cZo30tum584zrGATJ3). MIIA CENTER (A5pMESW...) NO usa V2.
// Cada bloque envuelto en try/catch — si V2 falla, V1 sigue como hoy.
const { resolveV2ChatType, loadVoiceDNAForGroup } = require('../core/voice_v2_loader');
...
const V2_OWNER_PERSONAL_UID = 'bq2BbtCVF8cZo30tum584zrGATJ3';
...
MIIA, genera tu respuesta breve, estratégica y humana:`;
  ...
  // ═══════════════════════════════════════════════════════════════
  // V2 UPSTREAM (C-386) — inyectar DNA lingüístico del subregistro
  // SOLO MIIA Personal (V2_OWNER_PERSONAL_UID). MIIA CENTER NO entra acá.
  // try/catch defensivo: si falla, V1 sigue como antes.
  // ═══════════════════════════════════════════════════════════════
  let v2ChatType = null;
  let v2VoiceBlock = null;
  let v2Subregistro = null;
  if (uid === V2_OWNER_PERSONAL_UID) {
    try {
...
- JAMÁS mencionar "me pidieron", "mi jefe", "el owner"`;
  ...
  // ═══════════════════════════════════════════════════════════════
  // V2 DOWNSTREAM (C-386) — auditor 10 red flags + regenerate-once + fallback §8
  // SOLO MIIA Personal y SOLO si UPSTREAM cargó DNA (v2ChatType !== null).
  // try/catch defensivo: si falla, V1 ya pulió y aiMessage queda intacto.
  // ═══════════════════════════════════════════════════════════════
  if (uid === V2_OWNER_PERSONAL_UID && v2ChatType) {
    try {
  ...
  // ═══════════════════════════════════════════════════════════════
  // V2 DOWNSTREAM (C-386) — splitter por subregistro + emoji injector
  // SOLO MIIA Personal y SOLO si UPSTREAM cargó DNA (v2ChatType !== null).
