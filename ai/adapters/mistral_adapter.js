...
async function call(apiKey, prompt, opts = {}) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    const warningTimer = setTimeout(() => {
      console.warn(`[MISTRAL] ⚠️ fetch lleva ${FETCH_WARNING_MS/1000}s sin respuesta (timeout en ${(timeoutMs - FETCH_WARNING_MS)/1000}s más)`);
    }, FETCH_WARNING_MS);
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        console.error(`[MISTRAL] ⏱️ TIMEOUT ${timeoutMs/1000}s — abort attempt ${attempt + 1}/${MAX_RETRIES}`);
      }
...
async function callChat(apiKey, messages, systemPrompt, opts = {}) {
  ...
  const controller = new AbortController();
  const warningTimer = setTimeout(() => {
    console.warn(`[MISTRAL] ⚠️ chat fetch lleva ${FETCH_WARNING_MS/1000}s sin respuesta (timeout en ${(timeoutMs - FETCH_WARNING_MS)/1000}s más)`);
  }, FETCH_WARNING_MS);
  const abortTimer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    ...
    if (error.name === 'AbortError') {
      console.error(`[MISTRAL] ⏱️ TIMEOUT ${timeoutMs/1000}s — chat aborted`);
    }
